...
exports.FragmentUtils = void 0;
class FragmentUtils {
    // Pattern to match fragment markers
    static FRAGMENT_PATTERN = /\/\/ ==== YOUR CODE: @(.*?) ====(.*?)\/\/ ==== END YOUR CODE ====/gs;
    // Comment styles for different languages
    static COMMENT_STYLES = {
     ...
     */
    static generateMarkerInsertion(request) {
     ...
     */
    static hasOverlappingFragments(content, insertPosition) {
        let match;
     ...
     */
    static findFragmentMarkers(content) {
        const markers = [];
        let match;
        const regex = new RegExp(this.FRAGMENT_PATTERN);
        while ((match = regex.exec(content)) !== null) {
     ...
     */
    static extractIndentation(lineContent) {
     ...
     */
    static parseFragmentsWithLines(content) {
     ...
     */
    static findNestedFragments(content) {
     ...
     */
    static replaceFragmentContent(fileContent, fragmentId, newContent) {
...
exports.FragmentUtils = FragmentUtils;
